...
| Field | Details |
|---|---|
| **Student Name** | *Nilesh Yadav* |
| **Registration Number** |  *24BCE11057*|
| **Chosen Software** | Git |
...
OS_Code/
├── README.md                          ← This file
├── script1_system_identity_final.sh
│── script2_package_inspector_final.sh
│── script3_disk_permission_auditor_final.sh 
│── script4_log_analyzer_final.sh 
│── script5_manifesto_generator_final.sh
└── OSS_Audit_Git_Report_final.docx           ← Project report (also submitted on portal)
```
...
**File:** `script1_system_identity_final.sh`
...
**File:** `script2_package_inspector_final.sh`
...
**File:** `script3_disk_permission_auditor_final.sh`
...
**File:** `script4_log_analyzer_final.sh`
...
**File:** `script5_manifesto_generator_final.sh`
...
```bash
git clone https://github.com/Nilesh-Yadav11/Open_linux_vityarthi
cd OS_Code
```
...
```bash
sudo ./script3_disk_permission_auditor_final.sh
```
...
./script4_log_analyzer_final.sh /var/log/syslog

# With a custom keyword
./script4_log_analyzer_final.sh /var/log/syslog WARNING

# On Fedora/CentOS — use messages instead
./script4_log_analyzer_final.sh /var/log/messages error
```
